function showMessage(message, isError = false) {
    ...
    if (isError) modalBody.style.color = 'red';
    else modalBody.style.color = 'black';
...
function getPartidos() {
    fetch('/api/partidos').then((response)=>response.json()).then((data)=>{
        ...
        data.forEach((partido)=>{
            ...
            switch(partido.resultado){
                ...
                case -1:
                    ...
                    break;
                case 0:
                    resultado = "Esperando finalizaci\xf3n";
                    break;
            }
            const deporteEmoji = {
                futbol: "\u26BD",
            ...
            let botonesAccion = `
                ...
                `;
            if (partido.resultado === 0) botonesAccion += `
...
function abrirModalEditar(id) {
    fetch(`/api/partidos/${id}`).then((response)=>response.json()).then((data)=>{
...
function deletePartido(id) {
    fetch(`/api/partidos/${id}`, {
        ...
        if (response.ok) {
            ...
            getPartidos();
        } else showMessage('Error al eliminar el partido.', true);
    }).catch((error)=>showMessage('Error al eliminar el partido: ' + error.message, true));
}
function ganarPartido(id) {
    fetch(`/api/partidos/${id}/gana`, {
        method: 'POST'
    }).then((response)=>{
        if (!response.ok) return response.text().then((errorMessage)=>{
...
function downloadFile(id) {
    fetch(`/api/partidos/download?id=${id}`).then((response)=>{
        if (!response.ok) return response.text().then((errorMessage)=>{
            throw new Error(errorMessage);
        });
